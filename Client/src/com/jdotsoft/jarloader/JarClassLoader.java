 ...
 * for the class or resource to create. The nested JARs could be located
...
public class JarClassLoader extends ClassLoader {
     ...
     * Default value is {@link LogLevel}.
     */
    ...
    public JarClassLoader(ClassLoader parent) {
        ...
        if ("http".equals(protocol) || "https".equals(protocol)) {
            ...
            try {
                ...
                logError(LogArea.JAR, "Failure to create HTTP JAR: %s %s", urlTopJar, e.toString());
                return;
            }
        }
     ...
     * 1. There are no ways to create JAR defined in a JarEntry directly
    ...
    private JarEntryInfo findJarNativeEntry(String sLib) {
        ...
        for (JarFileInfo jarFileInfo : this.lstJarFile) {
            ...
            while (en.hasMoreElements()) {
                ...
                //   - in the path: abc/Native.dll/xyz/my.dll <-- do not create this one!
                //   - in the partial name: abc/aNative.dll   <-- do not create this one!
     ...
     *
     * @param sClassName class to create.
     * @return Loaded class.
     * @throws JarClassLoaderException
     */
    ...
    private void persistNewTemp(File fileCfg) {
        ...
        logDebug(LogArea.CONFIG, "Persisting %d temp file names into %s",
                this.hsDeleteOnExit.size(), fileCfg.getAbsolutePath());
    ...
    public String getManifestMainClass() {
        ...
        if (isLaunchedFromJar()) {
            try {
                ...
                Manifest m = this.lstJarFile.get(0).jarFile.getManifest();
     ...
     * able to create a class from a JAR without calling findClass().
     */
    @Override
    protected synchronized Class<?> loadClass(String sClassName, boolean bResolve)
        ...
        // to create the class from rt.jar.
        ...
        // 5. The class loader which is used to create the requested component is
        ...
        // 6. The system class loader is used to create requested component if
        ...
        try {
            ...
            // If it called BEFORE it will create class-in-jar using
            // SystemClassLoader and "infect" it with SystemClassLoader.
            // The SystemClassLoader will be used to create all dependent
            // classes. SystemClassLoader will fail to create a class from
            // jar-in-jar and to create dll-in-jar.
            try {
            ...
            // What else?
            throw new ClassNotFoundException("Failure to create: " + sClassName);
        } finally {
            if (c != null && bResolve) {
                resolveClass(c);
            }
        ...
        if (isLaunchedFromJar()) {
            ...
            if (inf != null) {
                try {
                    ...
                    logError(LogArea.NATIVE, "Failure to create native library %s: %s", sLib, e.toString());
                }
            }
            return null;
        }
    ...
    /**
     * The system class loader could create resources defined as
    ...
    private void log(LogLevel level, LogArea area, String sMsg, Object... obj) {
        if (level.ordinal() <= this.logLevel.ordinal()) {
            if (this.hsLogArea.contains(LogArea.ALL) || this.hsLogArea.contains(area)) {
                this.logger.printf("JarClassLoader-" + level + ": " + sMsg + "\n", obj);
            }
        }
        if (!this.bLogConsole && level == LogLevel.ERROR) { // repeat to console
    ...
    private static class JarEntryInfo {
         ...
         * a helper method to create JAR entry into temporary file.
         *
